...
class Verlet(Slide):
    def construct(self):
        ...
        a_eq = MathTex(
            r"\mathbf{a}^i_{n+1}",
            r" =",
            r"\frac{\sum_{j \neq i} \mathbf{F}_{LJ}(|\mathbf{r}^i_{n+1}-\mathbf{r}^j_{n+1}|)}{m_i}",
        )
        a_eq[0].set_color(pink)
        a_eq[2][10:15].set_color(teal)
        a_eq.move_to(v_eq)
        a_eq.scale(0.7)
        ...
        self.play(
            r_vt.animate.set_value(-1.7),
            a_vt.animate.set_value(-3),
            v_vt.animate.set_value(-0.5),
            n_vt.animate.set_value(2),
        )

        self.play(
            r_vt.animate.set_value(-2.0),
            a_vt.animate.set_value(-1),
            v_vt.animate.set_value(-1.5),
            n_vt.animate.set_value(3),
        )

        self.play(
            r_vt.animate.set_value(-4.0),
            a_vt.animate.set_value(0.8),
            v_vt.animate.set_value(0.5),
            n_vt.animate.set_value(4),
        )

        self.play(
            r_vt.animate.set_value(-3.8),
            a_vt.animate.set_value(1.3),
            v_vt.animate.set_value(0.2),
            n_vt.animate.set_value(5),
        )

        self.play(
            r_vt.animate.set_value(-3.4),
            a_vt.animate.set_value(0.8),
            v_vt.animate.set_value(0.7),
            n_vt.animate.set_value(6))


        self.next_slide()
        text_3d = Tex("3D").move_to(top_left,aligned_edge=LEFT)
        self.wipe(Group(atom_i,atom_j,v_i,v_j,a_i,a_j,num_line,num_line_label,atom_i_lab,atom_j_lab,algo_text[0]),text_3d)

        self.next_slide()
        self.play(FadeOut(text_3d))
